...
__version__ = "0.8.0+jeff47"

# --- Compatibility imports for pymodbus 2.x and 3.x ---

# Endian moved from constants → payload in 3.x
try:
    from pymodbus.constants import Endian  # pymodbus < 3.0
except ImportError:
    from pymodbus.payload import Endian  # pymodbus >= 3.0

# --- Normalize Endian attributes (BIG/LITTLE vs Big/Little) ---
if hasattr(Endian, "Big"):  # pymodbus >= 3.0
    Endian.BIG = Endian.Big
    Endian.LITTLE = Endian.Little


# BinaryPayloadBuilder and BinaryPayloadDecoder remained in payload,
# so these are safe across versions.
from pymodbus.payload import BinaryPayloadBuilder, BinaryPayloadDecoder

# Modbus clients changed module paths in 3.x
try:
    # pymodbus >= 3.0
    from pymodbus.client import ModbusTcpClient, ModbusSerialClient
except ImportError:
    # pymodbus 2.x
    from pymodbus.client.sync import ModbusTcpClient, ModbusSerialClient # type: ignore[import]

# ReadHoldingRegistersResponse moved in 3.x
try:
    from pymodbus.register_read_message import ReadHoldingRegistersResponse  # 2.x
except ImportError:
    from pymodbus.pdu import ModbusResponse as ReadHoldingRegistersResponse  # 3.x

# --- Optional: version guard & logger setup ---
if version.parse(pymodbus.__version__) < version.parse("2.5.3"):
    raise ImportError("solaredge_modbus requires pymodbus >= 2.5.3")
...
class SolarEdge:
    ...
    def __str__(self):
        """User-friendly summary string for normal output."""
        if self.mode == connectionType.RTU:
            return f"{self.model} @ {self.device} (unit={self.unit})"
        elif self.mode == connectionType.TCP:
            return f"{self.model} @ {self.host}:{self.port} (unit={self.unit})"
        else:
            return f"{self.model} (unit={self.unit})"

    def _read_registers(self, address, length, func_name, human_label):
        """Internal helper for register reads with full error/reconnect logic."""
        ...
        if not func:
            log.debug(f"Client has no method {func_name}")
            return None

        try:
            result = func(address, length, slave=self.unit)
            if hasattr(result, "registers") and result.registers:
                ...
                log.debug(f"No response/empty {human_label} data at 0x{address:04X} — skipping block")
                return None

        except Exception as e:
            log.debug(f"Error reading {human_label} 0x{address:04X}: {e}")
            # Retry once if the last success was long ago
            if not getattr(self, "last_success", 0) or (time.time() - self.last_success > 10):
                log.debug(f"Attempting reconnect after {human_label} failure")
                self.disconnect()
                self.connect()
            return None

    def _read_holding_registers(self, address, length):
        """Read a holding register range; skip quickly if unsupported."""
        return self._read_registers(address, length, "read_holding_registers", "holding")

    def _read_input_registers(self, address, length):
        """Read an input register range (read-only telemetry)."""
        return self._read_registers(address, length, "read_input_registers", "input")
    ...
    def connected(self):
        """Check if Modbus client socket is open (compatible with pymodbus 2.x and 3.x)."""
        if not self.client:
            return False
    ...
    def read_all(self, rtype=registerType.HOLDING):
        """Read all known registers of the given type efficiently, with clear context warnings."""
        registers = {k: v for k, v in self.registers.items() if v[2] == rtype}
        results = {}
        any_success = False
        unsupported_seen = False
        self.last_success = getattr(self, "last_success", 0)

        context = self.__class__.__name__.lower()

        if not hasattr(self, "_warned_contexts"):
            # global tracker for all read_all calls this session
            self._warned_contexts = set()

        for batch in range(1, len(registers) + 1):
            register_batch = {k: v for k, v in registers.items() if v[7] == batch}
            if not register_batch:
                break

            t0 = time.time()
            batch_result = self._read_all(register_batch, rtype)
            t1 = time.time()
            ...
            if batch_result:
                ...
                log.debug(f"{context}: Batch {batch} read OK ({len(batch_result)} items in {t1 - t0:.2f}s)")
            else:
                if hasattr(self, "_unsupported_blocks") and self._unsupported_blocks:
                    unsupported_seen = True
                log.debug(f"{context}: Batch {batch} had no valid data (took {t1 - t0:.2f}s)")

        # --- summarize ---
        if not any_success and isinstance(self, Inverter) and rtype == registerType.HOLDING:
            log.warning(f"{context}[unit={self.unit}]: No successful reads — inverter may be offline")

        elif unsupported_seen:
            missing = ", ".join([f"0x{a:04X}" for a in sorted(self._unsupported_blocks)])
            log.warning(
                f"{context}[unit={self.unit}]: One or more queried registers unsupported ({missing})"
            )

        elif not any_success:
            # Try to extract richer context info for user debugging
            base = getattr(self, "base_addr", None) or getattr(self, "start", None)
            model = getattr(self, "model_id", None) or getattr(self, "model", None)
            serial = getattr(self, "serial", None) or getattr(self, "serialnumber", None)

            # Optional friendly names for well-known SunSpec models
            model_names = {
                101: "Single Phase Inverter",
                102: "Split Phase Inverter",
                103: "Three Phase Inverter",
                111: "Meter",
                112: "AC Meter",
                124: "Battery Storage (Legacy)",
                160: "Battery Storage (Enhanced)",
            }

            parts = [f"unit={self.unit}"]
            if model:
                friendly = model_names.get(model)
                if friendly:
                    parts.append(f"model={model} ({friendly})")
                else:
                    parts.append(f"model={model}")
            if base:
                parts.append(f"base=0x{base:04X}")
            if serial:
                parts.append(f"serial={serial}")

            extrastr = "[" + ", ".join(parts) + "]"
            log.info(
                f"{context}{extrastr}: No successful {rtype.name} reads "
                f"(Likely not supported by inverter)"
            )
